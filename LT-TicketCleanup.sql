...
#You should run these two again after completing the other steps
...
#Delete unsynced tickets that are older than 10 days
SELECT COUNT(*) FROM `tickets` WHERE ExternalID=0 AND UpdateDate < DATE_SUB(NOW(), INTERVAL 10 DAY);
DELETE FROM `tickets` WHERE ExternalID=0 AND UpdateDate < DATE_SUB(NOW(), INTERVAL 10 DAY);
